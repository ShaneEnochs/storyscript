...
window._ssReady = (async () => {
  try {
    ...
    const res = await fetch('./storyscript-engine.js');
    ...
    // (new Function wraps in a function scope but engine IIFE uses 'global'
    //  argument which is passed as 'this' — we pass window explicitly)
    const fn = new Function('global', src.replace(
