...
(function() {
  ...
  resizer.addEventListener('mousedown', (e) => {
    ...
    document.body.style.userSelect = 'none';
    // Disable pointer events on the iframe so mousemove/mouseup
    // are never swallowed when the cursor enters the preview pane.
    if (iframe) iframe.style.pointerEvents = 'none';
    resizer.classList.add('dragging');
    e.preventDefault();
  });

  // capture:true ensures mouseup fires even if cursor is over the iframe
  window.addEventListener('mousemove', (e) => {
    if (!dragging) return;
    const delta = startX - e.clientX;
    const MIN_RIGHT  = 220;
    // Available width minus a minimum editor width of 200px
    const maxRight = layout.offsetWidth - 200;
  ...
  }, { capture: true });  // capture so iframe can't swallow the event
})();
...
// ── StoryScript syntax highlighting via ViewPlugin + Decorations ─────────────
// CM6 StreamLanguage.token() must return strings, but those strings only produce
// colored spans if wired through a styleTags/HighlightStyle pipeline that requires
// @lezer/highlight's internal node type system — incompatible with our line-based
// tokenizer. The reliable alternative is a ViewPlugin that walks the visible lines,
// runs our regex tokenizer, and applies Decoration.mark() spans directly.
// This is independent of the parser pipeline and always works.

// CSS classes applied by the decoration marks (defined in ide.css):
//   .ss-kw    blue   bold  — *commands
//   .ss-cmt   gray   italic — *comment lines
//   .ss-opt   purple       — \t#choice options
//   .ss-str   green        — "string literals"
//   .ss-num   red          — numbers
//   .ss-var   orange       — ${interpolations}
//   .ss-label purple bold  — label names after *label/*goto/*gosub

// Tokenise a single line and return an array of {from, to, cls} spans.
// `lineStart` is the absolute document offset of the line's first character.
function tokeniseLine(lineText, lineStart) {
  const spans = [];
  const push = (from, to, cls) => spans.push({ from: lineStart + from, to: lineStart + to, cls });

  let i = 0;
  const len = lineText.length;

  // Choice option line: optional tabs then #
  if (/^\t+#/.test(lineText)) {
    push(0, len, 'ss-opt');
    return spans;
  }

  // Skip leading whitespace — commands inside *if/*choice blocks are indented with tabs
  const leadingWS = lineText.match(/^[\t ]*/)[0].length;
  i = leadingWS;

  // Command line: optional leading whitespace then *word
  const cmdMatch = lineText.slice(i).match(/^\*(\w+)/);
  if (cmdMatch) {
    const cmdStart = i;
    const cmdEnd   = i + cmdMatch[0].length;
    const cmd = cmdMatch[1].toLowerCase();

    // *comment — colour entire line as comment
    if (cmd === 'comment') {
      push(0, len, 'ss-cmt');
      return spans;
    }

    push(cmdStart, cmdEnd, 'ss-kw');
    i = cmdEnd;

    // Label definition: *label <n>
    if (cmd === 'label') {
      const rest = lineText.slice(i);
      const lm = rest.match(/^\s+(\w+)/);
      if (lm) push(i + lm[0].indexOf(lm[1]), i + lm[0].length, 'ss-label');
      return spans;
    }

    // Label reference: *goto / *gosub
  ...
  // Scan the rest of the line for strings, numbers, interpolations
  while (i < len) {
    // ${...} interpolation
    ...
    if (lineText[i] === '"' || lineText[i] === "'") {
      const q = lineText[i];
      let j = i + 1;
      while (j < len && lineText[j] !== q) j++;
      push(i, j + 1, 'ss-str');
      i = j + 1;
      continue;
    }
    // Numbers (standalone, not mid-word)
    const numMatch = lineText.slice(i).match(/^-?\d+(\.\d+)?/);
    if (numMatch && (i === 0 || /\W/.test(lineText[i - 1]))) {
      push(i, i + numMatch[0].length, 'ss-num');
      i += numMatch[0].length;
      continue;
    }
    i++;
...
// Build the ViewPlugin extension.
// buildSSHighlight() returns an array: [ViewPlugin extension, theme extension]
function buildSSHighlight(CM) {
  const { ViewPlugin, Decoration, RangeSetBuilder, EditorView } = CM;

  // Pre-build mark decorations for each class (reused across updates)
  const marks = {};
  for (const cls of ['ss-kw','ss-cmt','ss-opt','ss-str','ss-num','ss-var','ss-label']) {
    marks[cls] = Decoration.mark({ class: cls });
  }
  ...
  function buildDecorations(view) {
    ...
    for (const { from, to } of view.visibleRanges) {
      ...
      for (let ln = startLine.number; ln <= endLine.number; ln++) {
        ...
        const spans = tokeniseLine(line.text, line.from);
        for (const { from: f, to: t, cls } of spans) {
          if (marks[cls]) builder.add(f, t, marks[cls]);
        }
      }
    }
  ...
  const ssHighlightPlugin = ViewPlugin.fromClass(
    class {
  ...
  // Theme just handles editor chrome colours — token colours come from ide.css .ss-* rules
  ...
  return [ssHighlightPlugin, ssTheme];
}
...
function initCodeMirror(CM) {
  ...
  // buildSSHighlight returns [ViewPlugin extension, theme extension]
